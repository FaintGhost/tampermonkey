...
- **自动跳过**：取消（Canceled）/ 未完成（Unfulfilled）的行程不产生票据，批量下载自动跳过；页面上这类卡片也会直接隐藏，不占位
...
3. **批量下载**：页面右下角出现「Uber 票据批量下载」面板，按需调整日期范围（默认最近 30 天）。面板可按住标题栏拖拽移动，点「—」可收起成小条（位置和收起状态会记住，刷新不丢）
